...
from transformer_watermarking.encoder_decoder import *
from transformer_watermarking.Discriminator import Discriminator
...
class Network:
	...
	def __init__(self, H, W, message_length, noise_layers, device, batch_size, lr, with_diffusion=False,
		...
		self.encoder_decoder = EncoderDecoder(noise_layers)
		self.discriminator = Discriminator().to(device)

		self.encoder_decoder = torch.nn.DataParallel(self.encoder_decoder, device_ids=[0])
		self.discriminator = torch.nn.DataParallel(self.discriminator, device_ids=[0])
	...
	def train(self, images: torch.Tensor, messages: torch.Tensor):
		...
		with torch.enable_grad():
			...
			flops, params = profile(self.encoder_decoder, inputs=(images, messages,))
	...
	def decoded_message_error_rate_Precision(self, message, decoded_message):
		length = message.shape[0]
		message = message.gt(0.5)
		decoded_message = decoded_message.gt(0.5)
		sum = 0
		...
		for i in range(length):
			if decoded_message[i] == 1:
				len += 1
				if message[i] != 1:
					sum += 1
		error_rate = float(sum) / len
		return error_rate

	def decoded_message_error_rate_Recall(self, message, decoded_message):
		length = message.shape[0]
		message = message.gt(0.5)
		decoded_message = decoded_message.gt(0.5)
		sum = 0
	...
	def decoded_message_error_rate_NC(self, message, decoded_message):
		length = message.shape[0]
		message_cpu = message.cpu()
		decoded_message_cpu = decoded_message.cpu()
		message = np.array(message_cpu)
		decoded_message = np.array(decoded_message_cpu)
		message = np.where(message > 0.5, 1, 0)
		decoded_message = np.where(decoded_message_cpu > 0.5, 1, 0)
		message_men = np.mean(message)
		decoded_message_men = np.mean(decoded_message)
		numerator = np.sum((message - message_men) * (decoded_message - decoded_message_men))
		denominator = np.sqrt(np.sum((message - message_men) ** 2) * np.sum((decoded_message - decoded_message_men) ** 2))
	...
	def decoded_message_error_rate_batch(self, messages, decoded_messages):
		...
		for i in range(batch_size):
			error_rate += self.decoded_message_error_rate_NC(messages[i], decoded_messages[i])
